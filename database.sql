...
-- Tue 07 Apr 2020 12:36:29 AM CET
...
CREATE TABLE IF NOT EXISTS `birtouta`.`order_products` (
  ...
  `note` TEXT NULL,
  `photo` VARCHAR(255) NULL,
  PRIMARY KEY (`id`, `id_order`, `id_category`),
  CONSTRAINT `fk_order_product_order1`
    FOREIGN KEY (`id_order`)
    REFERENCES `birtouta`.`order` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_order_products_product_category1`
    FOREIGN KEY (`id_category`)
    REFERENCES `birtouta`.`product_category` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `birtouta`.`configs`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `birtouta`.`configs` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `last_version` VARCHAR(45) NULL,
  `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NULL,
  `id_user` INT NOT NULL,
  PRIMARY KEY (`id`, `id_user`),
  CONSTRAINT `fk_configs_users1`
    FOREIGN KEY (`id_user`)
    REFERENCES `birtouta`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
